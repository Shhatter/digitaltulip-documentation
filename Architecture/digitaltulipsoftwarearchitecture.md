...
[High Level Interaction Diagram](/Images/HighLevelInteractionDiagram.png)
...
The user submits their Active Directory credentials and becomes authenticated redirecting them back to the Portal home page
The passport-saml now allows the user to through as it has an authentication token
Express returns the single page app and this renders on the users screen.
Angular makes two calls in parallel. 
The first call is to retrieve the applications a user has access to.
A call is made to the Authorisation service to retrieve authorised applications
The application details for the authorised applications are retrieved from the mongo data store and rendered on the screen by angular
Simultaneously a call is made to retrieve the News items. 
The express server side code calls ServiceNow for news items
This is returned to angular and rendered on the screen.
